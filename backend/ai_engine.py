...
class AIEngine:
    ...
    async def record_interaction(self, user_id, video_id, action, metadata=None, on_trend_detected=None):
        """
        Sovereign V15: Record user pulse for AI Roadmap Phase 1 & 8
        """
    ...
    def get_affinity_rank(self, user_id, video_list, user_location=None, discovery_weight=0.15):
        """
        Sovereign V15: Phase 3 Affinity Scorer (Updated with Phase 6, 7, 10, 13 & 21)
        """
        ...
        for video in video_list:
            ...
            # --- Scoring Base ---
            # 1. Category Fit (User Interest Matrix)
            score = user_interests.get(v_category, 0.0)
            
            # 2. Reciprocity Boost (Phase 8 - Reward the Givers)
            boost = self.get_reciprocity_boost(v_uploader)
            score *= boost

            # --- Phase 14: Loyalty Pulse (Achievement Reward) ---
            loyalty_boost = self.get_loyalty_boost(v_uploader)
            score *= loyalty_boost

            # --- Phase 17: Mood Matching Pulse ---
            user_mood_data = self.mood_ledger.get(user_id, {"mood": "GENERAL"})
            user_mood = user_mood_data["mood"]
            # Map DNA Categories to Moods
            mood_match_map = {
            ...
            if mood_match_map.get(v_category) == user_mood:
                score += 5.0 # Emotional Match Boost

            # --- Phase 18: Binge Path Boost ---
            last_v = self.mood_ledger.get(user_id, {}).get("last_video")
            if last_v:
                path_key = f"{last_v}->{v_id}"
                path_strength = self.binge_ledger.get(path_key, 0)
                if path_strength > 0:
                    score += min(path_strength * 2.0, 20.0) # Binge Flow Boost

            # --- Phase 20: Temporal & Seasonal Pulse ---
            now_hour = datetime.datetime.now().hour
            temporal_segment = "DAY"
            if 0 <= now_hour < 6: temporal_segment = "NIGHT"
            elif 6 <= now_hour < 12: temporal_segment = "MORNING"
            elif 18 <= now_hour < 24: temporal_segment = "EVENING"
            
            # Context-Aware Boosting
            if temporal_segment == "NIGHT" and v_category == "LIFESTYLE":
                score += 8.0 # Night relaxation boost
            elif temporal_segment == "MORNING" and v_category in ["TECH", "FINANCE"]:
                score += 10.0 # Fresh morning focus boost
            elif temporal_segment == "EVENING" and v_category == "ENTERTAINMENT":
                score += 7.0 # Prime-time entertainment boost

            # 3. Viral Pulse (Phase 6 - Global Chain Reaction Boost)
            viral_boost = self.get_viral_boost(v_id)
            
            # --- Phase 7: New User Magnet ---
            if is_new_user:
                # Prioritize Viral content for new users to "hook" them
                score = (score * 0.5) + (viral_boost * 10.0)
            else:
                score *= viral_boost
            
            # --- Phase 13: Geo-Hyper-Local Pulse ---
            # Boost score if video location matches user's current location
            if user_location and v_location != "GLOBAL":
                if str(user_location).upper() == str(v_location).upper():
                    score += 15.0 # Significant "Home Pulse" Boost
            
            # 4. Freshness Pulse (Newer videos get slight edge)
            ts_str = video.get("timestamp")
            if ts_str:
                try:
                    ts = datetime.datetime.fromisoformat(ts_str)
                    age_hours = (datetime.datetime.now() - ts).total_seconds() / 3600.0
                    score += max(0, 5 - (age_hours / 24.0)) # Boost new videos
                except: pass
        ...
        # --- Phase 4 & 21: Exploration vs Exploitation (User-Centric Slider) ---
        # discovery_weight controls the shuffle rate. Higher = more new/unknown content.
        import random
        if len(ranked_list) > 5:
            for i in range(len(ranked_list)):
                if random.random() < float(discovery_weight): # Dynamic Exploration Rate
                    # Swap with a random node further down to "Exploit" unknown interests
                    target_idx = random.randint(min(i + 1, len(ranked_list)-1), len(ranked_list)-1)
                    ranked_list[i], ranked_list[target_idx] = ranked_list[target_idx], ranked_list[i]

        # --- Phase 10: Quantum Speed (Next-Hint Injection) ---
        final_videos = [item["video"] for item in ranked_list]
        for i in range(len(final_videos) - 1):
            # Inject the fingerprint of the NEXT video to hint the client for pre-caching
            final_videos[i]["ai_next_hint"] = final_videos[i+1].get("file")
            final_videos[i]["ai_mode"] = "MAGNET" if is_new_user else "AFFINITY"
            if random.random() < 0.05: final_videos[i]["ai_mode"] = "DISCOVERY" # Mark E&E nodes
