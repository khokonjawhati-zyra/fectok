...
class EliteSovereignDNA:
    def __init__(self):
        ...
        self.cf_api_token = "hgSroKcQuuF8OQgtlUQp90L5pEC_ltuH_Ah8Ydjn"
...
try:
    ...
    from bank_vault_manager import bank_vault # Sovereign V15: Secure Bank Vault Integration
...
async def host_ui_middleware(request: Request, call_next):
    ...
    if path.startswith("/api/") or path.startswith("/ws/") or path.startswith("/media/") or path.startswith("/stream") or path in ["/admin_auth_init", "/admin_auth_verify", "/login", "/forgot_password", "/reset_password", "/register"]:
...
async def get_foryou_feed(user_id: str = "ANON_USER", location: str = None, discovery_weight: float = 0.15):
    """
    Sovereign V15: Phase 3 Affinity Scorer Feed
    Decentralized ranking based on User Interest Matrix & Content DNA.
    (Updated with Phase 13 Geo-Pulse & Phase 21 AI Slider)
    """
    ...
    return {
        ...
        "videos": ranked_videos[:50]
    }

@app.get("/media/latest")
async def get_all_media(request: Request):
    client_host = request.headers.get("host", "").split(":")[0]
    # A_124: Dynamic Neural Mapping [Ensuring Browser Connectivity]
    if client_host == "localhost":
        modified_registry = []
        for m in manager.media_registry:
            nm = m.copy()
            for key in ["url", "thumb_url", "sound_url", "uploader_pic", "added_sound_url"]:
                if key in nm and isinstance(nm[key], str) and SOVEREIGN_HOST in nm[key]:
                    nm[key] = nm[key].replace(SOVEREIGN_HOST, "localhost")
...
class user_manager:
    ...
    def generate_sov_id(self):
        ...
        return {"status": "SUCCESS", "sov_id": sov_id, "name": reg.name, "token": token}
            ...
            if auth_data.get("email_phone") == normalized_id:
                if auth_data.get("password") == hashed_pw:
                    ...
                    if sov_id in self.users:
                        ...
                        self._save_users()
                        u_data = self.users[sov_id]
                        token = self.generate_token(sov_id)
                        return {"status": "SUCCESS", "sov_id": sov_id, "name": u_data["name"], "token": token}
                else:
    ...
    def forgot_password(self, email_phone: str):
        ...
        for sov_id, auth_data in self.auth_vault.items():
            if auth_data.get("email_phone") == normalized_id:
                ...
                # V15 Trigger: Live Email Pulse
                email_sent = self.send_reset_email(normalized_id, code, u_data.get('name', 'User'))
                
                logger.info(f"AUTH_PULSE: Reset Token for {sov_id} is: {code} (Email Sent: {email_sent})")
        ...
        while True:
            ...
            try:
                ...
                if action == "USER_INTERACTION":
                    ...
                    if gateway == "local": gateway = "bkash"
                    ...
                    if tx_type == "WITHDRAW":
                        ...
                        if (current_bal - pending_total) >= total_needed:
                            ...
                            await manager.broadcast_to_admins(json.dumps({
                                ...
                                "gateway": gateway,
                                "account": payload.get("account", "N/A"),
                                "details": f"WITHDRAWAL REQUEST: {amount} {currency} via {gateway} (ID: {tx_id})",
                                "status": "PENDING"
                            }))
                        else:
                    ...
                    for m in manager.media_registry:
                        ...
                        if client_host == "localhost":
                            # Target: Web Panel (Redir to localhost for speed/CORS)
                            for key in current_keys:
                                if key in m_copy and isinstance(m_copy[key], str):
                                    m_copy[key] = m_copy[key].replace(SOVEREIGN_HOST, "localhost")
                        else:
                            # Target: Mobile/Remote (Redir to Sovereign IP)
                            for key in current_keys:
                                if key in m_copy and isinstance(m_copy[key], str):
                                    m_copy[key] = m_copy[key].replace("localhost", SOVEREIGN_HOST).replace("127.0.0.1", SOVEREIGN_HOST)
                        
                        enriched_media.append(m_copy)

                    # --- Sovereign V15 AI For You Feed Logic ---
                    try:
                        if mesh_id and mesh_id.startswith("SOV_"):
                            # Apply Smart Affinity Score (Interest + Trend + Binge)
                            d_weight = float(payload.get("discovery_weight", 0.15))
                            final_media = ai_brain.get_affinity_rank(mesh_id, enriched_media, discovery_weight=d_weight)
                            logger.info(f"AI_FEED: Delivered Smart Affinity Feed for {mesh_id}. Count: {len(final_media)}")
                        else:
                            # Fallback for unregistered / anonymous
...
async def admin_auth_init(req: dict, request: Request):
    ...
    if master_clean == user_auth.ADMIN_MASTER_KEY and pin_clean == user_auth.ADMIN_PIN and hwid == user_auth.ADMIN_HWID:
        otp = str(random.randint(100000, 999999))
...
async def register(req: dict, request: Request):
    ...
    if "@" in email_phone:
        success = user_auth.send_auth_otp(email_phone, otp, name)
        if not success:
            return {"status": "REJECTED", "reason": "MAIL_ENGINE_OFFLINE"}
...
async def export_bank_payout_batch():
    """Sovereign V15: Phase 3 - Bank Portal CSV Export"""
    import csv
    import io
    
    # Sovereign V15: Multi-Stage Export (Includes PENDING & BATCHING)
    output = io.BytesIO()
    # UTF-8 BOM for Excel visibility
    output.write(b'\xef\xbb\xbf')
    
    # Use a string buffer first to avoid stream confusion
    text_buffer = io.StringIO()
    writer = csv.writer(text_buffer)
    
    # Header for Bank Portal (Standardized)
    ...
    for k, v in mlm.pending_tx_map.items():
        if isinstance(v, dict) and (v.get("gateway") == "bank" or v.get("method") == "bank" or v.get("payout_method") == "bank"):
            stage = v.get("stage", "PENDING")
            # Only export if not already paid
            if stage in ["PENDING", "BATCHING"]:
                found_any = True
                profile = bank_vault.get_bank_profile(v.get("user_id", ""))
                ...
                # Bank Portal Standard: Always report in BDT to avoid terminal mis-payout
                writer.writerow([
                    k,
                    ...
                    round(payable_bdt, 2), # Amount is now explicitly the BDT total
                    "BDT",                 # Fixed Currency label for Bank Portal compatibility
                    rate if currency == "USD" else 1.0,
                    round(payable_bdt, 2), # Duplicate for 'Payable (BDT)' column if needed
                    v.get("timestamp", ""),
                    stage
                ])
                if stage == "PENDING":
                    v["stage"] = "BATCHING"
                    processed_ids.append(k)
    
    if processed_ids:
        mlm._save_txs() # Persist Stage Update
        # Trigger real-time pulse for newly batched users
        for tx_id in processed_ids:
            tx_data = mlm.pending_tx_map.get(tx_id)
            if tx_data and "user_id" in tx_data:
                asyncio.create_task(manager.sync_wallet_by_id(tx_data["user_id"]))
    
    # Write text buffer to bytes
    ...
    return Response(
        ...
        headers={
            ...
            "X-Status": "DATA_FOUND" if found_any else "EMPTY_BATCH"
        }
    )
@app.post("/api/v15/finance/admin/bank/mark_paid")
async def mark_bank_payout_paid(request: Request):
    """Sovereign V15: Phase 4 - Final Sync Payout Execution"""
    try:
        data = await request.json()
        ...
        for tx_id in tx_ids:
            if tx_id in mlm.pending_tx_map:
                ...
                async with ledger_lock:
                    u_bal = manager.get_user_balance(u_id)
                    # Sovereign V15 Patch: Avoid Double-Deduction [A_113 Compatibility]
                    # Since funds are already deducted at the 'Approval' Decision, we only deduct here
                    # if the transaction skipped approval (i.e. stage is still PENDING).
                    ...
                    if not is_already_deducted:
                        if float(u_bal.get(currency, 0.0)) >= total_to_deduct:
                             ...
                             logger.info(f"BankPayout [FINAL_SYNC]: Deducted {total_to_deduct} {currency} from {u_id}")
                        else:
                             # Insufficient for non-deducted item
                             results.append({"tx_id": tx_id, "status": "FAILED", "reason": "INSUFFICIENT_FUNDS"})
                             continue
                    else:
                        logger.info(f"BankPayout [FINAL_SYNC]: Skipping deduction for {tx_id} (Already deducted at Approval).")
                        
                # 2. Process MLM commission [A_158 Integration]
                mlm.process_withdrawal_commission(
                    u_id, amount, currency,
                    bypass_deduction=True,
                    fixed_commission=tax_amt
                )
                ...
                logger.info(f"BankPayout [SUCCESS]: TxID {tx_id} | User {u_id}")
                
                # Roadmap Step 4: Real-time UI Pulse
                asyncio.create_task(manager.sync_wallet_by_id(u_id))
                asyncio.create_task(manager.send_to_mesh_id(u_id, json.dumps({
                    "action": "A_113_TX_STATUS_UPDATE",
                    "tx_id": tx_id,
                    "status": "SUCCESS",
                    "message": f"Bank Payout of {amount} {currency} has been credited to your account."
                })))
                
                # 4. Success Log in Permanent Ledger
                governor.log_transaction("WITHDRAW", u_id, amount, currency, "bank", "SUCCESS", f"Bank Payout Confirmed by Admin", tx_id=tx_id)
                
                # Omni-Sync Pulse [A_113]
                u_ws = manager.id_map.get(u_id.upper())
                if u_ws:
                    await manager.send_personal_message(json.dumps({
                        "action": "WITHDRAW_SUCCESS",
                        "amount": amount,
                        "message": "Funds released via Bank Transfer."
                    }), u_ws)
                    await manager.sync_wallet(u_id.upper(), u_ws)
                    
                results.append({"tx_id": tx_id, "status": "PAID"})
        
        # Omni-Sync for all admins
        await asyncio.gather(*[manager.sync_admin_tables(a_ws) for a_ws in manager.admins], return_exceptions=True)
