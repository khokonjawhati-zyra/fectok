...
class EliteSovereignDNA:
    ...
    def __init__(self):
        ...
        if IS_DOCKER:
            ...
            self.storage = "/var/www/html/media/videos" # Aligned with Backend & Nginx
...
def upload_video():
    ...
    uploader = request.form.get('uploader', 'ANON_USER')
    ...
    def process_async_mixing():
        ...
        try:
            ...
            print(f"[Uplink Hub] Backend Synchronization Locked.")
...
def get_latest_video():
    ...
    # Sovereign V15: High-Precision Production URL resolution
    PROTOCOL = "https" if IS_DOCKER or os.getenv("SYSTEM_MODE") == "PRODUCTION" else "http"
    PORT_INFIX = "" if PROTOCOL == "https" else ":8080"
    PROXY_PATH = "video_stream/stream" if PROTOCOL == "https" else "stream"
    
    return jsonify({
        "file": latest_file, 
        "url": f"{PROTOCOL}://{SOVEREIGN_HOST}{PORT_INFIX}/{PROXY_PATH}/{latest_file}"
    })
